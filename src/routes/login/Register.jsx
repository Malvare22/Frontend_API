...
const PanelPrincipal = (props) => {
    ...
    const handleSubmit = () => {
        ...
        if(rol == 'Estudiante'){
            ...
            axios.post('http://150.136.248.85:8080/register/estudiante/codigo', formData, config)
            .then((response) => {
        ...
        if(rol == 'Docente'){
            ...
            formData.append('contrasenia', contrasenia);
            axios.post('http://150.136.248.85:8080/register/docente/correo', formData, config)
            .then((response) => {
                alert("Se ha creado exitosamente su usuario, ya puede iniciar sesión con su código institucional y contraseña.")
                navigate('/Login');
            })
