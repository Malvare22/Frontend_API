...
export default function Listar_Ideas() {
    ...
    const getIdeas = async () => {
        let value = null;
        value = await axios.get("http://150.136.248.85:8080/ideaNegocio", { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
...
function Getdocentes() {
    ...
    const getDocentes = async () => {
        let value = null;
        value = await axios.get('http://150.136.248.85:8080/docente/listar', { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
                const data = response.data;
                return data;
