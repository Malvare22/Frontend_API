...
export default function Listar_Planes() {
    ...
    const getPlanes = async () => {
        const value = await axios.get("http://144.22.32.132:8080/planNegocio", { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
    ...
    const handleFilter = async (filters) => {
        ...
        try {
            ...
            let value = await axios.post("http://144.22.32.132:8080/planNegocio/filtrar", formData, config
            ).then(
                response => {
                    const data = response.data;
                    console.log(data);
