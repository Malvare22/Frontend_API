...
const Table = (props) => {
    ...
    const sortData = () => {
        ...
        return props && props.data.slice().sort((a, b) => {
            let comparison = 0;
            if (column === 'Entidad') {
                comparison = a && a.nombre.localeCompare(b && b.nombre);
            }
            if (!ascending) {
                comparison *= -1;
            }
    ...
    const toggleA = (correo) => {
        localStorage.setItem('id_entidad', correo);
        navigate('/Lider/VistaEntidades');
    };
    const toggleB = (datos) => {
        localStorage.setItem('id_entidad', JSON.stringify(datos));
        navigate('');
    };
    const eliminar = async (correo) => {
        try {
            ...
            await axios.delete("http://localhost:8080/entidadFinanciadora", config)
            props.updater();
            toggleAlert(null)
        }
        catch (error){
    ...
    return (
        <Sdiv>
                            ...
                            <tr key={d && d.id}>
                                ...
                                <td className='text-center align-middle'>
                                    <div>
                                        ...
                                        <button type="button" id="eliminar" value={d.id} onClick={() => toggleAlert({ correo: d && d.correo, entidad: d && d.nombre })} className="btn" style={{ width: "auto", border: "none" }}>
            ...
            <Modal isOpen={state} style={modalStyles}>
                <ModalBody>
                    <FormGroup>
                        <Label id="texto">¿Está seguro de que desea eliminar la entidad financiadora {valor.entidad}?</Label>
                    </FormGroup>
                </ModalBody>
                <ModalFooter>
                    <Button color="danger" onClick={() => eliminar(valor.correo)}>Deshabilitar</Button>
...
export default function Listar_Entidades() {
    ...
    const getEntidades = async () => {
        const value = await axios.get("http://localhost:8080/entidadFinanciadora", { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
