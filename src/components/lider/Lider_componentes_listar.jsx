...
export default function Listar_Componentes() {
    ...
    const getComponentes = async () => {
        const value = await axios.get("", { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
    ...
    return (
        <Sdiv>
                ...
                <table className="table table-striped">
                    ...
                    <tbody>
                        {sortedData && sortedData.map((d) => (
                            <tr key={d.id}>
                                <td className='text-center align-middle col-auto'>{ }</td>
                                <td className='text-center align-middle col-auto'>{ }</td>
                                <td className='text-center align-middle col-auto'>{ }</td>
                                <td className='text-center align-middle'>
                                    <div>
                                        <button type="button" className="btn" value={d.titulo} style={{ width: "auto", border: "none" }}>
            ...
            <Modal isOpen={state} style={modalStyles}>
                <ModalBody>
                    <FormGroup>
                        <Label id="texto">¿Está seguro de que desea eliminar el componente: {valor.titulo} que tiene como id: {valor.id}?</Label>
                    </FormGroup>
                </ModalBody>
                <ModalFooter>
