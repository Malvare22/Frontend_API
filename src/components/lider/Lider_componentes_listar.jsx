...
export default function Listar_Componentes() {
    ...
    const getComponentes = async () => {
        const value = await axios.get("http://129.151.121.230:8080/componenteCompetencias", { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
...
const Table = (props) => {
    ...
    const eliminar = async (nombre) => {
        ...
        try {
            ...
            await axios.delete(`http://129.151.121.230:8080/componenteCompetencias/${nombre}`, config)
            props.updater();
            toggleAlert(null)
        }
        catch (error) {
