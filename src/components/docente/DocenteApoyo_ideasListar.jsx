...
export default function Listar_Ideas() {
    ...
    const getIdeas = async () => {
        ...
        let value = await axios.post("http://129.151.121.230:8080/ideaNegocio/IdeasDocentesApoyo", formData, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
    ...
    const handleFilter = async (filters) => {
        ...
        try {
            ...
            const value = await axios.post("http://129.151.121.230:8080/ideaNegocio/IdeasDocentesApoyo",  formData, config
            ).then(
                response => {
                    const data = response.data;
                    console.log(data)
