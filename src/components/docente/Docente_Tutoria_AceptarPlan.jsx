...
export default function Aceptar_TutoriaPlan() {    
    ...
    const aceptar = async (tituloPlan) => {
        try {
            await axios.get(`http://localhost:8080/docente/acceptarTutorPlan/${tituloPlan}/true`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
    ...
    const rechazar = async (tituloPlan) => {
        try {
            await axios.get(`http://localhost:8080/docente/acceptarTutorPlan/${tituloPlan}/false`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
            navigate('/Docente/Perfil');
        } catch (error) {
            console.log(error);
        }
