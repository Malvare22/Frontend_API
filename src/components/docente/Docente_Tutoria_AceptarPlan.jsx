...
export default function Aceptar_TutoriaPlan() {    
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://146.235.246.199:8080/docente/${jsonObject.email}`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
    ...
    const aceptar = async (tituloPlan) => {
        try {
            await axios.get(`http://146.235.246.199:8080/docente/acceptarTutorPlan/${tituloPlan}/true`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
    ...
    const rechazar = async (tituloPlan) => {
        try {
            await axios.get(`http://146.235.246.199:8080/docente/acceptarTutorPlan/${tituloPlan}/false`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
            navigate('/Docente/Perfil');
        } catch (error) {
            console.log(error);
        }
