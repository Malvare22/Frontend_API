...
export default function Aceptar_Tutoria() {    
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get(`http://129.151.121.230:8080/docente/${jsonObject.email}`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
    ...
    const aceptar = async (titulo) => {
        try {
            console.log(titulo)
            await axios.get(`http://129.151.121.230:8080/docente/acceptarTutor/${titulo}/true`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
    ...
    const rechazar = async (titulo) => {
        try {
            await axios.get(`http://129.151.121.230:8080/docente/acceptarTutor/${titulo}/false`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
            navigate('/Docente/Perfil');
        } catch (error) {
            console.log(error);
        }
