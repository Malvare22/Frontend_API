...
export default function Aceptar_Tutoria() {    
    ...
    const aceptar = async (titulo) => {
        try {
            console.log(titulo)
            await axios.get(`http://144.22.63.128:8080/docente/acceptarTutor/${titulo}/true`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
    ...
    const rechazar = async (titulo) => {
        try {
            await axios.get(`http://144.22.63.128:8080/docente/acceptarTutor/${titulo}/false`, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } });
            navigate('/Docente/Perfil');
        } catch (error) {
            console.log(error);
        }
