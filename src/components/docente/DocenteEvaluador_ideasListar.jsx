...
export default function Listar_Ideas() {
    ...
    const getIdeas = async () => {
        ...
        let value = await axios.post("http://144.22.32.132:8080/ideaNegocio/IdeasDocentesEvaluadores", formData, { headers: { "X-Softue-JWT": localStorage.getItem("token_access") } }
        ).then(
            response => {
    ...
    const handleFilter = async (filters) => {
        ...
        try {
            ...
            const value = await axios.post("http://144.22.32.132:8080/ideaNegocio/IdeasDocentesEvaluadores",  formData, config
            ).then(
                response => {
                    const data = response.data;
                    console.log(data)
